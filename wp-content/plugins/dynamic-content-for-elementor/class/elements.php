...
class Elements
{
    ...
    public function init()
    {
        if (!is_admin()) {
            ...
            if (current_user_can('administrator') || isset($_GET['dce-nav'])) {
        ...
        ?></div>
				...
				<div id="elementor-navigator__elements">
					<div data-model-cid="c44" class="elementor-navigator__element elementor-navigator__element--has-children">
						<?php 
        $this->print_elementor_navigator(self::$elementor_data, 0, current_user_can('administrator'));
        ?>
					</div>
				</div>
			</div>
        ...
        if (!empty($elementor_data)) {
            if (\is_array($elementor_data)) {
                ...
                foreach ($elementor_data as $ekey => $evalue) {
                    ...
                    if (!current_user_can('administrator') && isset(self::$elements_hidden[$id]) && empty(self::$elements_hidden[$id]['fallback'])) {
                        continue;
                    }
                    ?>

					<li class="elementor-navigator__element elementor-navigator__element-<?php 
                    echo $type . (\is_array($evalue) ? ' elementor-navigator__element--has-children' : '') . (current_user_can('administrator') && isset(self::$elements_hidden[$id]) && empty(self::$elements_hidden[$id]['fallback']) ? ' elementor-navigator__element--hidden' : '');
                    ?>">
						<div class="elementor-navigator__item elementor-active<?php 
                    echo current_user_can('administrator') && isset(self::$elements_hidden[$id]) ? ' elementor-dce-hidden' : '';
                    ?>" data-target="<?php 
                    echo $target;
                    ?>">
					<?php 
                    ...
                    if (!current_user_can('administrator') || !isset(self::$elements_hidden[$id])) {
                    ...
                    ?>
										...
										<?php 
                    if (isset(self::$elements_hidden[$id]) && current_user_can('administrator')) {
                        ?>
