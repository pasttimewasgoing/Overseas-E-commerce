...
trait Strings
{
    ...
    public static function tablefy($html = '')
    {
        ...
        $dom->loadStr($html);
        // A helper that, given a Tag instance and a new tag name, uses reflection
        // to override its protected $name property.
        $setTagName = function ($node, $newName) {
            $reflector = new \ReflectionClass($node);
            $prop = $reflector->getProperty('name');
            $prop->setAccessible(\true);
            $prop->setValue($node, $newName);
        };
        // Replace container ➔ table
        foreach ($dom->find('.elementor-container') as $wrapper) {
            $setTagName($wrapper->tag, 'table');
        }
        // Replace row ➔ tr
        foreach ($dom->find('.elementor-row') as $row) {
            $setTagName($row->tag, 'tr');
        }
        // Replace column ➔ td
        foreach ($dom->find('.elementor-column') as $col) {
            $setTagName($col->tag, 'td');
        }
        return (string) $dom;
    }
    //+exclude_end
