...
class FileBrowser extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_register_controls()
    {
        ...
        // Removed legacy Root Folder/SubFolder controls previously used for 'uploads'
                ...
                if (!Helper::is_empty_dir($fulldir) || $settings['empty']) {
                    if (!$hide || \Elementor\Plugin::$instance->editor->is_edit_mode()) {
                        ...
                        if ($customTitle) {
                            ...
                            echo esc_url($this->path_to_url($fulldir));
                            ?>" target="_blank">
							<?php 
                        } else {
                            ?>
                ...
                if (\Elementor\Plugin::$instance->editor->is_edit_mode() || !$hide) {
                    ...
                    if ($customTitle) {
                        ...
                        if (!empty($settings['enable_metadata']) && $settings['enable_metadata_wp_title'] && $post_id) {
                            ...
                            echo '<input type="text" class="dce-file-title" data-md5="' . $md5 . '"' . ($post_id ? ' data-post-id="' . $post_id . '"' : '') . ' name="dce-file-browser[' . $md5 . '][title]" value="' . esc_attr($title) . '" />';
                        }
                        ...
                        if (isset($settings['enable_metadata_description']) && $settings['enable_metadata_description']) {
                            ...
                            if (\Elementor\Plugin::$instance->editor->is_edit_mode()) {
                                if ($settings['enable_metadata_wp_description'] && $post_id) {
                                    ...
                                    echo '<textarea class="dce-file-description block" data-md5="' . $md5 . '"' . ($post_id ? ' data-post-id="' . $post_id . '"' : '') . ' name="dce-file-browser[' . $md5 . '][description]">' . esc_textarea($description) . '</textarea>';
                                }
                            } elseif (\trim($description)) {
                                echo '<div class="dce-file-description block">' . wp_kses_post($description) . '</div>';
                            }
                        }
                    }
                    echo '</li>';
